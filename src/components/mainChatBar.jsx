...
const MainChatBar = () => {
  ...
  const handleSubmit = (e) => {
    ...
    socket.emit('newMessage', { message, auth: userId.username });
    setMessage('');
  };
    ...
    <Col className="p-0 h-100">
      <Col md="auto" className="d-flex flex-column h-100">
        ...
        <Container id="messages-box" className="chat-messages overflow-auto px-5 ">
          {storeMessages.map((el) => <Message key={el.id} user={el.auth} message={el.message} />)}
        </Container>
        <Container className="mt-auto px-5 py-3">
          <Form noValidate="" className="py-1 border rounded-2" onSubmit={handleSubmit}>
            <Container className="input-group has-validation p-0">
              <Form.Control name="body" aria-label="Новое сообщение" placeholder="Введите сообщение..." className="border-0 p-0 ps-2 form-control" value={message} onChange={messageChange} />
